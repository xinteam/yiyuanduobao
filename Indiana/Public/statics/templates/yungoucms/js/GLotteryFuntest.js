function gg_show_Time_fun(times,objc,data){
    var b=GG_SHOP_TIME.path;
    ...
    if(time<=0){
        ...
        setTimeout(function(){
            lastgetlotteryer(obj,data['id']);
        },5000);
        return;
    }

    setTimeout(function(){
        gg_show_Time_fun(times,objc,data);
    },30);

}
//倒计时完成后最后返回获奖人信息
function lastgetlotteryer(obj,id)
{
    $.get(GG_SHOP_TIME.path+"/api/getshoptest/checkshowtime/"+new Date().getTime(),{'gid':id},function(indexData){
        ...
        if(info.error == '0' && info.id != 'null'){
            ...
            uhtml += '<dl>';
            uhtml += '<dt>';
            uhtml += '<a href="'+GG_SHOP_TIME.path+'/dataserver/'+info.id+'.html" target="_blank" class="pic">';
            ...
            uhtml+='</dd>';
            uhtml+='<br />';
            uhtml+='<dd class="u-name"><a href="'+GG_SHOP_TIME.path+'/dataserver/'+info.id+'.html" target="_blank" class="name">'+info.title+'</a></dd>';
            uhtml+='</dl>';
...
function gg_show_time_add_li(div,path,info){
    ...
    gg_show_Time_fun(info.times,div_li_obj,data);
}//

function gg_show_time_init(div,path,gid){
    window.setTimeout("gg_show_time_init()",8000);
    ...
    $.get(GG_SHOP_TIME.path+"/api/getshoptest/lottery_shop_json/"+new Date().getTime(),{'gid':GG_SHOP_TIME.gid},function(indexData){
        var info = jQuery.parseJSON(indexData);
